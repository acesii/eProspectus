		...
		<div id="list-subject" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="subjCode" title="${message(code: 'subject.subjCode.label', default: 'Subj Code')}" />
					
						<g:sortableColumn property="subjName" title="${message(code: 'subject.subjName.label', default: 'Subj Name')}" />
				...
				<g:each in="${subjectInstanceList}" status="i" var="subjectInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${subjectInstance.id}">${fieldValue(bean: subjectInstance, field: "subjCode")}</g:link></td>
					
						<td>${fieldValue(bean: subjectInstance, field: "subjName")}</td>
